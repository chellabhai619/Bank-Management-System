...
def crdt_write(master,amt,accnt,name,sub):
...
def debit_write(master,amt,accnt,name,sub):
...
def Cr_Amt(master,accnt,name):
    ...
    l1.pack(side="top")
    e1.pack(side="top")
    b=tk.Button(creditwn,text="Credit",relief="raised",command=lambda:crdt_write(creditwn,e1.get(),accnt,name,master))
    b.pack(side="top")
    creditwn.bind("<Return>",lambda x:crdt_write(creditwn,e1.get(),accnt,name))


def De_Amt(master,accnt,name):
    debitwn=tk.Tk()
    ...
    l1.pack(side="top")
    e1.pack(side="top")
    b=tk.Button(debitwn,text="Debit",relief="raised",command=lambda:debit_write(debitwn,e1.get(),accnt,name,master))
    b.pack(side="top")
...
def disp_bal(master,accnt):
    fdet=open(accnt+".txt",'r')
    fdet.readline()
    bal=fdet.readline()
    fdet.close()
    ba=tk.Label(text="Balance "+bal,relief="raised",bg="black",fg="white",anchor="center",justify="center",width='20',height='20')
    ba.place(height='50',width='120',x=710,y=180)
...
def logged_in_menu(accnt,name):
    ...
    b2=tk.Button(image=myimg2,command=lambda: Cr_Amt(rootwn,accnt,name))
    b2.image=myimg2
    b3=tk.Button(image=myimg3,command=lambda: De_Amt(rootwn,accnt,name))
    b3.image=myimg3
    disp_bal(rootwn,accnt)
